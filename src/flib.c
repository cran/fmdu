...
static double variance( const size_t n, const double* const a, const size_t inca )
...
// nonnegative power transformation: y-hat = a + b * x^c
{
  double xmin = DBL_MAX;
  if ( isnull( w ) ) {
    if ( symmetric == true ) { for ( size_t i = 2; i <= n; i++ ) for ( size_t j = 1; j < i; j++ ) if ( x[i][j] < xmin ) xmin = x[i][j]; }
    else { for ( size_t i = 1; i <= n; i++ ) for ( size_t j = 1; j <= n; j++ ) if ( i != j ) if ( x[i][j] < xmin ) xmin = x[i][j]; }
  }
  else {
    if ( symmetric == true ) { for ( size_t i = 2; i <= n; i++ ) for ( size_t j = 1; j < i; j++ ) if ( isnotzero( w[i][j] ) && x[i][j] < xmin ) xmin = x[i][j]; }
    else { for ( size_t i = 1; i <= n; i++ ) for ( size_t j = 1; j <= n; j++ ) if ( i != j ) if ( isnotzero( w[i][j] ) && x[i][j] < xmin ) xmin = x[i][j]; }
  }
  ...
  // lowest and highest x value needed for offset constraint(s)
  ...
  double bound = -1.0 * b * pow( xmin, c );
  if ( use_a && a < bound ) a = bound;
  ...
  if ( mxbeta > DBL_EPSILON ) {
    for ( size_t iter = 1; iter <= MAXITER; iter++ ) {
      ...
      for ( size_t inner = 1; inner <= MAXINNER; inner++ ) {
        ...
        bound = -1.0 * atry[2] * pow( xmin, atry[3] );              // bound depends on scale and exponent
        if ( use_a && atry[1] < bound ) atry[1] = bound;           // data cannot become negative
...
void nnboxcox( const bool symmetric, const size_t n, double** x, double** y, const double mconst, double** w, const bool use_a, const bool use_b, double** r )
...
static void knotsequenceinterval( double* boundaries, const size_t ninner, double* iknots )
{
  if ( ninner == 0 ) return;
  const double xmin = boundaries[1];
  const double xmax = boundaries[2];
...
static void getknotsequence( const size_t n, double* x, double* w, const size_t ninner, double* iknots, double* boundaries, const int knotstype )
{
  ...
  else if ( knotstype == KNOTSTYPE.INTERVAL ) knotsequenceinterval( boundaries, ninner, iknots );
...
int gethashmapint( const size_t mapsize, hashmap* map, char32 key, const int defint )
{
  #ifdef R
    return( 0 );
  #else
...
size_t gethashmapsize_t( const size_t mapsize, hashmap* map, char32 key, const size_t defsize_t )
{
  #ifdef R
    return( 0 );
  #else
